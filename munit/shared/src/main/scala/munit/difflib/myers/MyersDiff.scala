...
class MyersDiff[T](equalizer: Equalizer[T]) extends munit.difflib.DiffAlgorithm[T] {
  ...
  private def buildRevision(
    ...
    var path = _path
    val patch = new munit.difflib.Patch[T]
    ...
    while (path != null &&
      ...
      val delta: munit.difflib.Delta[T] =
        if (original.size == 0 && revised.size != 0) {
          ...
          new munit.difflib.DeleteDelta[T](original, revised)
        } else {
          new munit.difflib.ChangeDelta[T](original, revised)
        }
